...
const graph = NavGraph.build(world, bounds, doors, geometryHash(scene), undefined, message => console.log(message), raisedWays(scene, world))
...
console.log(`baked public/nav/compound.json in ${seconds.toFixed(1)} s: ${graph.nx} x ${graph.nz} spots + ${graph.raised.length} raised, ${walkable} walkable, `
  + `${data.links.length} links (${data.links.filter(l => l.kind === 'climb').length} climbs), ${sizes.length} regions (largest ${(Math.max(...sizes) / walkable * 100).toFixed(1)}%), `
